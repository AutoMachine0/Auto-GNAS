...
class MessagePassingNet(MessagePassing):
    ...
    def __init__(self,
        ...
        self.weight = Parameter(torch.Tensor(self.input_dim, heads * self.output_dim))
        glorot(self.weight)
    ...
    def forward(self, x, edge_index):
        ...
        x = torch.mm(x, self.weight).view(-1, self.heads, self.output_dim)
    ...
    def node_representation_transformer(self,node_representation):

        if self.concat is True:
            node_representation = node_representation.view(-1, self.heads * self.output_dim)
        else:
